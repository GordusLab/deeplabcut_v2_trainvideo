...
def identify_ufmf_version(filename):
    ...
    version_buf = fd.read( version_buflen ).decode('ascii')
    had_marker=False
    if version_buf=='ufmf':
...
def _write_dict(fd,save_dict):
    fd.write('d'.encode('ascii'))
    fd.write(chr(len(save_dict.keys())).encode('ascii'))
    ...
    for key in keys:
        ...
        if isinstance(value,list) or isinstance(value,np.ndarray):
            ...
            b = ('a' + dtype_char).encode('ascii') + struct.pack('<L',len(larr)*bytes_per_element)
            b += larr.tostring()
            fd.write(b)
            continue
...
def _read_array(fd,buf_remaining):
    ...
    # Ensure the same data type is used across platforms / NumPy installs
    dtype = np.int32 if dtype_char == b'l' else dtype_char
    ...
    larr = np.fromstring(data_buf, dtype=dtype)
...
class _UFmfV3LowLevelReader(object):
    ...
    def _read_keyframe_chunk(self):
        ...
        keyframe_type = self._fd_read(len_type).decode('ascii')
        ...
        dtype_char = dtype_char.decode('ascii')
    ...
    def get_next_frame(self, _return_more=False):
        ...
        for timestamp, regions in self._ufmf.readframes():
            if isinstance(self._ufmf,UfmfV1):
                ...
                except KeyError as NoMoreFramesException:
...
class UfmfSaverV3(UfmfSaverBase):
    ...
    def __init__(self, file,
        ...
        buf = struct.pack( FMT[self.version].HEADER, 'ufmf'.encode('ascii'),
                           self.version, 0, self.max_width, self.max_height,
                           len(self.coding) )
        self.file.write(buf)
        self.file.write(coding.encode('ascii'))
    ...
    def add_keyframe(self,keyframe_type,image_data,timestamp):
        ...
        try:
            ...
            raise
        b =  (chr(KEYFRAME_CHUNK) + chr(char2) + keyframe_type).encode('ascii') # chunkid, len(type), type
        b += struct.pack(FMT[self.version].KEYFRAME2,dtype.encode('ascii'),width,height,timestamp)
        loc = self.file.tell()
        tmp = self._index['keyframe'][keyframe_type]
        if len(tmp)==0:
            tmp['timestamp']=[]
        ...
        self.file.write(np_image_data)

    def _add_frame_regions(self,timestamp,regions):
        n_pts = len(regions)
        b = chr(FRAME_CHUNK).encode('ascii') + struct.pack(FMT[self.version].POINTS1, timestamp, n_pts)
        loc = self.file.tell()
        tmp = self._index['frame']
        if len(tmp)==0:
            tmp['timestamp']=[]
        ...
        for region in regions:
            ...
            str_buf.append( this_str_head + this_str_buf )
            #fullstr = str_buf[-1] if fullstr is None else fullstr + str_buf[-1]
        fullstr = b''.join(str_buf)
        self.file.write(fullstr)

    def _add_frame_regions_cache(fname, generator, version):
        import pickle
        from tqdm import tqdm_notebook as tqdm
        def _processRegions(regions):
            str_buf, tss = [], []
            for region in regions:
                ts,xmin,ymin,roi = region
                h,w = roi.shape
                this_str_buf = roi.tostring()
                assert len(this_str_buf)==w*h
                this_str_head = struct.pack(FMT[version].POINTS2, xmin, ymin, w, h)
                str_buf.append( this_str_head + this_str_buf )
                tss.append(ts)
            fullstr = b''.join(str_buf)
            return [tss, fullstr]

        # Now write each region to cache
        with open(fname, 'wb') as fout:
            receivedKeyframes = False
            for regions in tqdm(generator, leave=False):
                if not receivedKeyframes:
                    receivedKeyframes = True
                    keyframes = regions
                    pickle.dump(keyframes, fout)
                else:
                    n_pts = len(regions)
                    pickle.dump( tuple([n_pts,] + _processRegions(regions)), fout )

    def _add_frame_regions_fromcache(self, fnameCache, timestamp_offset):
        import pickle
        with open(fnameCache, 'rb') as f:
            keyframes = pickle.load(f)
            # Add keyframes
            for kf in keyframes:
                self.add_keyframe(kf[0], kf[1], kf[2] + timestamp_offset)
            # Add regions
            while True:
                # Load from cache
                try:
                    n_pts, _timestamps, fullstr = pickle.load(f)
                    _timestamp = _timestamps[0]
                except Exception as e:
                    break
                timestamp = _timestamp + timestamp_offset
                # Format timestamp
                b = chr(FRAME_CHUNK).encode('ascii') + \
                    struct.pack(FMT[self.version].POINTS1, timestamp, n_pts)
                # Update index
                loc = self.file.tell()
                tmp = self._index['frame']
                if len(tmp)==0:
                ...
                self.file.write(b)
                self.file.write(fullstr)
                # Send update that 1 frame was written
                yield True
    ...
    def close(self):
        b = chr(INDEX_DICT_CHUNK).encode('ascii')
        self.file.write(b)
        loc = self.file.tell()
